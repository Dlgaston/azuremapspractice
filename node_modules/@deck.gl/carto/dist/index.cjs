...
__export(dist_exports, {
  ...
  QuadbinHeatmapTileLayer: () => quadbin_heatmap_tile_layer_default,
...
var VERSION = true ? "9.0.16" : "latest";
...
var VERSION2 = true ? "9.0.16" : "latest";
...
// dist/layers/quadbin-heatmap-tile-layer.js
...
var _DrawCallbackLayer = class extends import_core8.Layer {
  initializeState() {
...
var DrawCallbackLayer = _DrawCallbackLayer;
(() => {
  _DrawCallbackLayer.layerName = "DrawCallbackLayer";
})();
function RTTModifier(BaseLayer) {
  var _a;
  return _a = class extends BaseLayer {
    draw(opts) {
...
function PostProcessModifier(BaseLayer, effect) {
  ...
  return _a = class extends BaseLayer {
    initializeState(context) {
      ...
      this.internalState.postProcess = new import_core8.PostProcessEffect(effect, this.props);
...
// dist/layers/quadbin-heatmap-tile-layer.js
var _RTTSolidPolygonLayer = class extends RTTModifier(import_layers2.SolidPolygonLayer) {
  getShaders(type) {
  ...
  draw(opts) {
    ...
    const cellResolution = Number((0, import_quadbin5.getResolution)(cell0.id));
...
var RTTSolidPolygonLayer = _RTTSolidPolygonLayer;
(() => {
  _RTTSolidPolygonLayer.layerName = "RTTSolidPolygonLayer";
})();
var PostProcessQuadbinTileLayer = PostProcessModifier(QuadbinTileLayer, heatmap);
function encodeWeight(w) {
...
var _QuadbinHeatmapTileLayer = class extends import_core9.CompositeLayer {
  renderLayers() {
    var _a, _b, _c;
    ...
    const getFillColor = typeof getWeight === "function" ? (d, info) => encodeWeight(getWeight(d, info)) : encodeWeight(getWeight);
    return new PostProcessQuadbinTileLayer(this.getSubLayerProps({
      id: "heatmap",
...
var QuadbinHeatmapTileLayer = _QuadbinHeatmapTileLayer;
(() => {
  _QuadbinHeatmapTileLayer.layerName = "QuadbinHeatmapTileLayer";
})();
(() => {
  _QuadbinHeatmapTileLayer.defaultProps = defaultProps6;
})();
var quadbin_heatmap_tile_layer_default = QuadbinHeatmapTileLayer;
...
// dist/layers/raster-layer.js
var defaultProps7 = {
  ...import_layers3.ColumnLayer.defaultProps,
  extruded: false,
  diskResolution: 4,
  vertices: [
    [-0.5, -0.5],
...
var _RasterColumnLayer = class extends import_layers3.ColumnLayer {
  getShaders() {
    const shaders = super.getShaders();
...
var _RasterLayer = class extends import_core10.CompositeLayer {
  renderLayers() {
...
(() => {
  _RasterLayer.defaultProps = defaultProps7;
})();
...
var _RasterTileLayer = class extends import_core11.CompositeLayer {
  getLoadOptions() {
    const loadOptions = super.getLoadOptions() || {};
    const tileJSON = this.props.data;
    injectAccessToken(loadOptions, tileJSON.accessToken);
...
(() => {
  _RasterTileLayer.defaultProps = defaultProps8;
})();
...
var VERSION3 = true ? "9.0.16" : "latest";
...
(0, import_core12.registerLoaders)([carto_properties_tile_loader_default, carto_vector_tile_loader_default]);
var defaultProps9 = {
...
var _VectorTileLayer = class extends import_geo_layers6.MVTLayer {
  ...
  renderSubLayers(props) {
    ...
    const subLayer = new import_layers4.GeoJsonLayer(subLayerProps);
    return subLayer;
  }
...
(() => {
  _VectorTileLayer.defaultProps = defaultProps9;
})();
...
var DEFAULT_PARAMETERS = {
  v: V3_MINOR_VERSION,
  deckglVersion: import_core13.VERSION
};
var DEFAULT_HEADERS = {
...
async function requestWithParameters({ baseUrl, parameters, headers: customHeaders, errorContext }) {
  ...
  const fetchPromise = url.length > MAX_GET_LENGTH ? fetch(baseUrl, { method: "POST", body: JSON.stringify(parameters), headers }) : fetch(url, { headers });
  let response;
...
function getLayer(type, config, dataset) {
  ...
  const layerTypeDefs = {
    point: {
      Layer: import_layers5.GeoJsonLayer,
      propMap: {
        columns: {
  ...
  return {
    ...
    defaultProps: { ...defaultProps10, ...layer.defaultProps }
  };
}
  ...
  return {
    ...
    layers: layers.reverse().map(({ id: id4, type, config, visualChannels }) => {
      try {
        ...
        const { Layer: Layer2, propMap, defaultProps: defaultProps11 } = getLayer(type, config, dataset);
        ...
        import_core14.log.error(e.message)();
        return void 0;
      }
    })
  };
